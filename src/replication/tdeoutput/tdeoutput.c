 ...
 *
 * IDENTIFICATION
 *		  src/replication/tdeoutput/tdeoutput.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
#include "replication/tdeoutput.h"

PG_MODULE_MAGIC;

static void tdeoutput_startup(LogicalDecodingContext *ctx,
							 OutputPluginOptions *opt, bool is_init);
static void tdeoutput_shutdown(LogicalDecodingContext *ctx);
static void tdeoutput_begin_txn(LogicalDecodingContext *ctx,
							   ReorderBufferTXN *txn);
static void tdeoutput_commit_txn(LogicalDecodingContext *ctx,
								ReorderBufferTXN *txn, XLogRecPtr commit_lsn);
static void tdeoutput_change(LogicalDecodingContext *ctx,
							ReorderBufferTXN *txn, Relation relation,
							ReorderBufferChange *change);
static void tdeoutput_truncate(LogicalDecodingContext *ctx,
							  ReorderBufferTXN *txn, int nrelations, Relation relations[],
							  ReorderBufferChange *change);
static void tdeoutput_message(LogicalDecodingContext *ctx,
							 ReorderBufferTXN *txn, XLogRecPtr message_lsn,
							 bool transactional, const char *prefix,
							 Size sz, const char *message);
static bool tdeoutput_origin_filter(LogicalDecodingContext *ctx,
								   RepOriginId origin_id);
static void tdeoutput_begin_prepare_txn(LogicalDecodingContext *ctx,
									   ReorderBufferTXN *txn);
static void tdeoutput_prepare_txn(LogicalDecodingContext *ctx,
								 ReorderBufferTXN *txn, XLogRecPtr prepare_lsn);
static void tdeoutput_commit_prepared_txn(LogicalDecodingContext *ctx,
										 ReorderBufferTXN *txn, XLogRecPtr commit_lsn);
static void tdeoutput_rollback_prepared_txn(LogicalDecodingContext *ctx,
										   ReorderBufferTXN *txn,
										   XLogRecPtr prepare_end_lsn,
										   TimestampTz prepare_time);
static void tdeoutput_stream_start(struct LogicalDecodingContext *ctx,
								  ReorderBufferTXN *txn);
static void tdeoutput_stream_stop(struct LogicalDecodingContext *ctx,
								 ReorderBufferTXN *txn);
static void tdeoutput_stream_abort(struct LogicalDecodingContext *ctx,
								  ReorderBufferTXN *txn,
								  XLogRecPtr abort_lsn);
static void tdeoutput_stream_commit(struct LogicalDecodingContext *ctx,
								   ReorderBufferTXN *txn,
								   XLogRecPtr commit_lsn);
static void tdeoutput_stream_prepare_txn(LogicalDecodingContext *ctx,
										ReorderBufferTXN *txn, XLogRecPtr prepare_lsn);
...
static RelationSyncEntry *get_rel_sync_entry(TDEOutputData *data,
											 Relation relation);
...
static bool get_schema_sent_in_streamed_txn(RelationSyncEntry *entry,
											TransactionId xid);
static void init_tuple_slot(TDEOutputData *data, Relation relation,
							RelationSyncEntry *entry);

/* row filter routines */
static EState *create_estate_for_relation(Relation rel);
static void tdeoutput_row_filter_init(TDEOutputData *data,
									 List *publications,
									 RelationSyncEntry *entry);
static bool tdeoutput_row_filter_exec_expr(ExprState *state,
										  ExprContext *econtext);
static bool tdeoutput_row_filter(Relation relation, TupleTableSlot *old_slot,
								TupleTableSlot **new_slot_ptr,
								RelationSyncEntry *entry,
								ReorderBufferChangeType *action);

/* column list routines */
static void tdeoutput_column_list_init(TDEOutputData *data,
									  List *publications,
									  RelationSyncEntry *entry);
...
{
	cb->startup_cb = tdeoutput_startup;
	cb->begin_cb = tdeoutput_begin_txn;
	cb->change_cb = tdeoutput_change;
	cb->truncate_cb = tdeoutput_truncate;
	cb->message_cb = tdeoutput_message;
	cb->commit_cb = tdeoutput_commit_txn;

	cb->begin_prepare_cb = tdeoutput_begin_prepare_txn;
	cb->prepare_cb = tdeoutput_prepare_txn;
	cb->commit_prepared_cb = tdeoutput_commit_prepared_txn;
	cb->rollback_prepared_cb = tdeoutput_rollback_prepared_txn;
	cb->filter_by_origin_cb = tdeoutput_origin_filter;
	cb->shutdown_cb = tdeoutput_shutdown;

	/* transaction streaming */
	cb->stream_start_cb = tdeoutput_stream_start;
	cb->stream_stop_cb = tdeoutput_stream_stop;
	cb->stream_abort_cb = tdeoutput_stream_abort;
	cb->stream_commit_cb = tdeoutput_stream_commit;
	cb->stream_change_cb = tdeoutput_change;
	cb->stream_message_cb = tdeoutput_message;
	cb->stream_truncate_cb = tdeoutput_truncate;
	/* transaction streaming - two-phase commit */
	cb->stream_prepare_cb = tdeoutput_stream_prepare_txn;
}

static void
parse_output_parameters(List *options, TDEOutputData *data)
{
	ListCell   *lc;
	...
	foreach(lc, options)
	{
		...
		else if (strcmp(defel->defname, "origin") == 0)
		{
			...
			elog(ERROR, "unrecognized tdeoutput option: %s", defel->defname);
	}
}
 ...
 */
static void
tdeoutput_startup(LogicalDecodingContext *ctx, OutputPluginOptions *opt,
				 bool is_init)
{
	TDEOutputData *data = palloc0(sizeof(TDEOutputData));
 ...
 */
static void
tdeoutput_begin_txn(LogicalDecodingContext *ctx, ReorderBufferTXN *txn)
{
 ...
 */
static void
tdeoutput_send_begin(LogicalDecodingContext *ctx, ReorderBufferTXN *txn)
{
 ...
 */
static void
tdeoutput_commit_txn(LogicalDecodingContext *ctx, ReorderBufferTXN *txn,
					XLogRecPtr commit_lsn)
{
	PGOutputTxnData *txndata = (PGOutputTxnData *) txn->output_plugin_private;
	bool		sent_begin_txn;
 ...
 */
static void
tdeoutput_begin_prepare_txn(LogicalDecodingContext *ctx, ReorderBufferTXN *txn)
{
	bool		send_replication_origin = txn->origin_id != InvalidRepOriginId;
 ...
 */
static void
tdeoutput_prepare_txn(LogicalDecodingContext *ctx, ReorderBufferTXN *txn,
 ...
 */
static void
tdeoutput_commit_prepared_txn(LogicalDecodingContext *ctx, ReorderBufferTXN *txn,
							 XLogRecPtr commit_lsn)
{
	OutputPluginUpdateProgress(ctx, false);
 ...
 */
static void
tdeoutput_rollback_prepared_txn(LogicalDecodingContext *ctx,
							   ReorderBufferTXN *txn,
							   XLogRecPtr prepare_end_lsn,
							   TimestampTz prepare_time)
{
 ...
 */
static bool
tdeoutput_row_filter_exec_expr(ExprState *state, ExprContext *econtext)
{
	Datum		ret;
	bool		isnull;
 ...
 */
static void
tdeoutput_ensure_entry_cxt(TDEOutputData *data, RelationSyncEntry *entry)
{
	Relation	relation;
 ...
 */
static void
tdeoutput_row_filter_init(TDEOutputData *data, List *publications,
						 RelationSyncEntry *entry)
{
	ListCell   *lc;
	...
	if (has_filter)
	{
		...
		tdeoutput_ensure_entry_cxt(data, entry);
 ...
 */
static void
tdeoutput_column_list_init(TDEOutputData *data, List *publications,
						  RelationSyncEntry *entry)
{
	ListCell   *lc;
	bool		first = true;
 ...
 */
static void
init_tuple_slot(TDEOutputData *data, Relation relation,
				RelationSyncEntry *entry)
{
	MemoryContext oldctx;
 ...
 */
static bool
tdeoutput_row_filter(Relation relation, TupleTableSlot *old_slot,
					...
					ReorderBufferChangeType *action)
{
	...
	if (!new_slot || !old_slot)
	{
		...
		result = tdeoutput_row_filter_exec_expr(filter_exprstate, ecxt);
	...
	old_matched = tdeoutput_row_filter_exec_expr(filter_exprstate, ecxt);
	...
	new_matched = tdeoutput_row_filter_exec_expr(filter_exprstate, ecxt);
 ...
 */
static void
tdeoutput_change(LogicalDecodingContext *ctx, ReorderBufferTXN *txn,
				Relation relation, ReorderBufferChange *change)
{
	TDEOutputData *data = (TDEOutputData *) ctx->output_plugin_private;
	PGOutputTxnData *txndata = (PGOutputTxnData *) txn->output_plugin_private;
	MemoryContext old;
	 ...
	 */
	if (!tdeoutput_row_filter(targetrel, old_slot, &new_slot, relentry, &action))
		goto cleanup;
...
static void
tdeoutput_truncate(LogicalDecodingContext *ctx, ReorderBufferTXN *txn,
				  int nrelations, Relation relations[], ReorderBufferChange *change)
{
	...
	/* Remember the xid for the change in streaming mode. See tdeoutput_change. */
	if (in_streaming)
		xid = change->txn->xid;
...
static void
tdeoutput_message(LogicalDecodingContext *ctx, ReorderBufferTXN *txn,
 ...
 */
static bool
tdeoutput_origin_filter(LogicalDecodingContext *ctx,
					   RepOriginId origin_id)
{
 ...
 */
static void
tdeoutput_shutdown(LogicalDecodingContext *ctx)
{
 ...
 */
static void
tdeoutput_stream_start(struct LogicalDecodingContext *ctx,
					  ReorderBufferTXN *txn)
{
	bool		send_replication_origin = txn->origin_id != InvalidRepOriginId;
 ...
 */
static void
tdeoutput_stream_stop(struct LogicalDecodingContext *ctx,
					 ReorderBufferTXN *txn)
{
 ...
 */
static void
tdeoutput_stream_abort(struct LogicalDecodingContext *ctx,
					  ReorderBufferTXN *txn,
					  XLogRecPtr abort_lsn)
{
 ...
 */
static void
tdeoutput_stream_commit(struct LogicalDecodingContext *ctx,
					   ReorderBufferTXN *txn,
					   XLogRecPtr commit_lsn)
{
	/*
 ...
 */
static void
tdeoutput_stream_prepare_txn(LogicalDecodingContext *ctx,
							ReorderBufferTXN *txn,
							XLogRecPtr prepare_lsn)
{
	Assert(rbtxn_is_streamed(txn));
...
get_rel_sync_entry(TDEOutputData *data, Relation relation)
{
	RelationSyncEntry *entry;
	bool		found;
	MemoryContext oldctx;
	...
	if (!entry->replicate_valid)
	{
			...
			entry->pubactions.pubdelete)
		{
			...
			/* Initialize the row filter */
			tdeoutput_row_filter_init(data, rel_publications, entry);

			/* Initialize the column list */
			tdeoutput_column_list_init(data, rel_publications, entry);
		}
